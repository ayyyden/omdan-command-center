...
export async function POST(req: NextRequest) {
  ...
  if (error) {
    if (error.code === "23505") return Response.json({ error: "Type already exists" }, { status: 409 })
...
export async function DELETE(req: NextRequest) {
  ...
  if (!itype) return Response.json({ error: "Not found" }, { status: 404 })
  if (itype.is_default) return Response.json({ error: "Built-in invoice types cannot be deleted" }, { status: 403 })

  // Soft-delete if already used, hard-delete otherwise
