...
function DatePickerModal({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-t-2xl w-full max-w-sm p-6 pb-10 shadow-2xl">
        <h3 className="text-sm font-semibold text-slate-900 mb-4">Select Date</h3>
        <input
          type="date"
          value={local}
          onChange={(e) => setLocal(e.target.value)}
          className="w-full border border-slate-300 rounded-lg px-3 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
          style={{ fontSize: "16px", color: "#0d0d1a" }}
        />
        <div className="flex gap-3 mt-4">
          ...
          <button
            type="button"
            onClick={() => { if (local) onChange(local); onClose() }}
            className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm font-semibold text-white transition-colors"
          >
            Done
          </button>
        </div>
...
// ── Date field (desktop: native picker, mobile: text + modal) ─────────────────
...
function FieldInput({
  ...
  const containerStyle: React.CSSProperties = {
    ...
    ...(process.env.NODE_ENV === "development"
      ? { outline: "1px dashed rgba(239,68,68,0.5)", outlineOffset: "-1px" }
      : {}),
  }
  ...
  switch (field.field_type) {
    case "text":
      return (
        <div style={containerStyle} title={field.label}>
          <input
            type="text"
            value={value}
            onChange={(e) => onChange(e.target.value)}
    ...
    case "checkbox":
      return (
        <div style={containerStyle} className="flex items-center justify-center" title={field.label}>
          <input
            type="checkbox"
            checked={value === "true"}
