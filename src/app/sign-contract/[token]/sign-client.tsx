...
export function SignClient({ token, contractName, pdfUrl, fields }: Props) {
  ...
  const { pages, loading: pdfLoading } = usePdfPages(pdfUrl, containerWidth)
            ...
            {pdfLoading ? (
              ...
              <div>
                {pages.map((pg) => {
                  ...
                  // Scale factor: screen pixels per PDF point — used to convert pt values to px for preview
...
function CanvasDisplay({ canvas }: { canvas: HTMLCanvasElement }) {
  ...
  return <div ref={ref} style={{ width: canvas.width, height: canvas.height }} />
}
