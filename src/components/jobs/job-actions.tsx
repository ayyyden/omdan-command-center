...
export function JobActions({ jobId, jobTitle, isArchived }: JobActionsProps) {
  ...
  async function handleArchive() {
    ...
    toast({ title: isArchived ? "Job restored" : "Job archived", description: jobTitle })
    router.push("/jobs")
    router.refresh()
  }
  ...
  async function handleDelete() {
    ...
    toast({ title: "Job deleted", description: `"${jobTitle}" has been permanently deleted.` })
    router.push("/jobs")
    router.refresh()
  }
      ...
      <Dialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        <DialogContent className="sm:max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            <span className="font-semibold text-foreground">"{jobTitle}"</span> and all its expenses
            and payments will be permanently deleted. This cannot be undone.
          </p>
